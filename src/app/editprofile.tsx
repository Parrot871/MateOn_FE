  ...
  useEffect(() => {
    getMyProfile()
      .then((profile) => {
        ...
        setPortfolioSummary(profile.portfolio ?? null);
      })
      .catch((error) => {
    ...
    <View className="flex-1 bg-white">
      ...
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView className="flex-1" contentContainerClassName="px-8 pb-10" keyboardShouldPersistTaps="handled">
          ...
          <Text className="text-black font-pretendard-semibold mb-2 text-lg">포트폴리오</Text>
          <TouchableOpacity
            onPress={handlePickPortfolio}
            disabled={isSummarizing}
            className="h-40 mb-8 px-4 bg-gray-50 rounded-xl border border-dashed border-[#3E6AF4]/50 justify-center items-center gap-1 active:bg-blue-50/30"
          >
            <Image source={PdfImg} style={{ width: 28, height: 28 }} />
            {isSummarizing ? (
              <Text className="text-[#3E6AF4] font-pretendard-semibold text-base text-center">AI가 포트폴리오를 분석하는 중...</Text>
            ) : portfolioFile ? (
              <Text className="text-gray-700 font-pretendard-semibold text-base text-center">{portfolioFile.name}</Text>
            ) : (
              <>
                <Text className="text-gray-700 font-pretendard-semibold text-base text-center">
                  PDF 포트폴리오를 선택해주세요
                </Text>
                <Text className="text-gray-400 font-pretendard text-xs text-center">최대 20MB</Text>
              </>
            )}
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            disabled={!isComplete || isSubmitting || isSummarizing}
            className={`h-14 rounded-xl justify-center items-center ${isComplete && !isSummarizing ? 'bg-[#3E6AF4]' : 'bg-[#3E6AF4]/40'}`}
          >
